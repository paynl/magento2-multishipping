...
namespace Paynl\Multishipping\Model;
    ...
    public function place(array $orderList): array
    {
        ...
        try {
            ...
            if ($methodInstance instanceof \Paynl\Payment\Model\Paymentmethod\Paymentmethod) {
                $this->paynlHelper->logNotice('Start new payment for multishipping order ' . $order->getId(), array(), $order->getStore());             
                $payPaymentCreate = (new PayPaymentCreate($order, $methodInstance));
                $payPaymentCreate->setAmount($totalOrderAmount);                
                if(!empty($this->getRedirectUrl($orderList))){
                    $payPaymentCreate->setFinishURL($this->getRedirectUrl($orderList));
                }
                $transaction = $payPaymentCreate->create();
